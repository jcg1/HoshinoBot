#TODO用config读取替换 “rex = re.compile(r'.*(ue|优衣).*')”方便更改
#用NIKENAME替换re.compile(r'.*(ue|优衣).*')为合适内容
import os
import random
...
async def check_limit_num(bot, ev: CQEvent):
    if lmtw.check(ev.user_id):
        await bot.send(ev, f'可以发优衣抽签来额外获取宝石哦', at_sender=True)
